...
/*   Updated: 2024/04/27 13:09:54 by ekose            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

static void	ft_mutex_unlock(t_philo *philo, char work)
{
	if (work == 'e')
	{
...
int	ft_wait(t_philo *philo, unsigned long time, char work)
{
	...
	while (ft_get_time() - now < time)
	{
			...
			&& !philo->data->stop)
		{
			...
			philo->data->stop = 1;
			philo->data->time_of_death = ft_get_time()
				- philo->data->start_time;
			ft_mutex_unlock(philo, work);
			return (0);
		}
