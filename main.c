...
/*   Updated: 2024/04/24 19:42:39 by ekose            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_multi_philo(t_philo *philo)
{
	while (philo->many_eat)
	{
		...
		printf ("%ld %d is eating\n",
			ft_get_time() - philo->data->start_time, philo->pos);
		if (ft_wait(philo, philo->data->time_to_eat, 'e') == 0)
			break ;
		...
		printf("%ld %d is sleeping\n",
			ft_get_time() - philo->data->start_time, philo->pos);
		if (ft_wait(philo, philo->data->time_to_sleep, 's') == 0)
			break ;
...
void	*ft_creat(t_data *data)
{
	...
	while (i < data->ttl_phl)
	{
		...
		if (pthread_create(&data->philos[i].td, NULL,
